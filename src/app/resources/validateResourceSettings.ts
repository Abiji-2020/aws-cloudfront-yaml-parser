  ...
  if (definition && definition.Settings) {
    ...
    Object.keys(definition.Settings).forEach((settingId) => {
      ...
      if (
        ...
        let valid = validateResourceSelectField(
          value,
          definition.Settings[settingId],
        );
        if (!valid) {
          let err: any = new Error(
            `Invalid setting in resource ${resource.Id}, value ${value} is an invalid choice for ${settingId}.`,
          );
